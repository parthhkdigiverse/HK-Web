...
export default function HomeSections({ overrideContent }) {
  ...
  const activeTestimonialsData = activeContent?.testimonials?.list || testimonialsData;

  useEffect(() => {
    if (activeTestimonialsData && activeTestimonial >= activeTestimonialsData.length) {
      setActiveTestimonial(0);
    }
  }, [activeTestimonialsData, activeTestimonial]);

  useEffect(() => {
    if (!activeTestimonialsData || !activeTestimonialsData.length || isTestimonialPaused) return;
    
    const interval = setInterval(() => {
      setActiveTestimonial((prev) => (prev + 1) % activeTestimonialsData.length);
    }, 6000); // 6 seconds auto-rotate
    
    return () => clearInterval(interval);
  }, [activeTestimonialsData, isTestimonialPaused]);

  const handlePrev = () => {
    if (!activeTestimonialsData.length) return;
    setActiveTestimonial((prev) => (prev - 1 + activeTestimonialsData.length) % activeTestimonialsData.length);
  };
  ...
  const getPosition = (index) => {
    ...
    if (!activeTestimonialsData.length) return 'right';
    if ((activeTestimonial - 1 + activeTestimonialsData.length) % activeTestimonialsData.length === index) return 'left';
    return 'right';
  };
          ...
          {visibleServices.map((service, idx) => {
            ...
            return (
              <a 
                ...
                onMouseEnter={() => setActiveImage(service.img || preview.img)}
              ...
              >
                ...
                <div 
                  className={`absolute bottom-0 left-0 w-0 h-[2px] bg-gradient-to-r ${service.gradient || preview.gradient} transition-all duration-500 group-hover:w-full z-10`}
                  style={{
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <h3 className="font-display text-lg font-bold text-white tracking-wide">{activeCaseStudy.client}</h3>
                      </div>
                    </div>
                    <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-white tracking-tight leading-tight">{activeCaseStudy.title}</h2>
                  </div>

                  {activeCaseStudy.points && activeCaseStudy.points.length > 0 && (
                    <div className="space-y-4 border-t border-white/5 pt-8">
                      ...
                      <ul className="space-y-3 font-light text-neutral-400 text-xs sm:text-sm">
                        {activeCaseStudy.points.map((point, index) => (
      ...
      {(() => {
        ...
        return (
          <section 
            ...
            className="py-32 px-6 sm:px-12 md:px-20 lg:px-28 xl:px-36 border-t border-b border-white/5 bg-black/30 relative overflow-hidden"
          ...
          >
            ...
            {activeTestimonialsData[activeTestimonial] && (
              <div 
                ...
                style={{
                  background: `radial-gradient(circle at 50% 50%, ${activeTestimonialsData[activeTestimonial].glowColor || 'rgba(255,255,255,0.05)'}, transparent 65%)`
                }}
              />
            )}
            ...
            <div className="max-w-[1600px] w-full mx-auto relative z-10">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-20">
                <div className="space-y-4">
                  ...
                  <h2 className="font-display text-3xl sm:text-4xl font-bold text-white tracking-tight">{activeTestimonialsObj.title}</h2>
                </div>
                <div className="max-w-xs text-neutral-400 font-light text-xs sm:text-sm leading-relaxed">
                  {activeTestimonialsObj.description}
                </div>
              </div>
              ...
              >
                {activeTestimonialsData.map((testimonial, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{
                        ...
                        boxShadow: isActive 
                          ? `0 25px 60px -15px ${testimonial.glowColor || 'rgba(255,255,255,0.1)'}` 
                    ...
                    >
                      {/* Glowing Accent */}
                      <div className={`absolute top-0 right-0 w-48 h-48 bg-gradient-to-br ${testimonial.color || 'from-white/5 to-white/0'} rounded-full filter blur-[60px] opacity-40 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none`} />
                      ...
                      <div className="flex justify-between items-start relative z-10">
                        ...
                        {testimonial.tag && (
                          <span className={`px-3.5 py-1 border rounded text-[9px] font-mono tracking-widest font-light transition-all duration-500 ${testimonial.tagClass || 'text-neutral-400 bg-white/5 border-white/10'}`}>
                            {testimonial.tag}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-white/5 relative z-10">
                        ...
                        <div className="flex gap-1">
                          {[...Array(testimonial.rating || 5)].map((_, i) => (
                            <svg key={i} className={`w-3.5 h-3.5 fill-current transition-colors duration-500 ${testimonial.starClass || 'text-yellow-500'}`} viewBox="0 0 20 20">
              ...
              <div className="flex justify-center items-center gap-6 mt-10">
                ...
                <div className="flex gap-2.5">
                  {activeTestimonialsData.map((_, i) => (
                    <button
                      key={i}
          ...
          <section id="bottom-cta-section" className="py-40 px-4 sm:px-6 md:px-8 lg:px-12 text-center max-w-[1600px] w-full mx-auto">
            <div className="space-y-10">
              <h2 className="font-display text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight max-w-4xl mx-auto leading-tight text-white">
                {activeBottomCta.titleNormal} <span className="text-neutral-400 font-light italic">{activeBottomCta.titleItalic}</span>
              </h2>
              <div className="flex justify-center pt-4">
                <a href={activeBottomCta.btnLink || "#contact"} className="glass-btn px-12 py-5 rounded-full text-xs uppercase tracking-[0.2em] font-medium text-white shadow-md inline-block">
                  {activeBottomCta.btnText || "Start a Project →"}
                </a>
              </div>
            </div>
          </section>
