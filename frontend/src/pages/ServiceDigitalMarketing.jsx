...
export default function ServiceDigitalMarketing({ overrideContent }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#030307] text-neutral-300 font-sans pb-24 overflow-x-hidden">
      ...
      {/* Background Ambience */}
      <div className="absolute top-[10%] left-1/4 w-[35vw] h-[35vw] rounded-full bg-amber-500/5 blur-[120px] pointer-events-none" />
      <div className="absolute top-[50%] right-1/4 w-[40vw] h-[40vw] rounded-full bg-orange-500/5 blur-[130px] pointer-events-none" />
      ...
      <section className="max-w-7xl mx-auto px-4 py-28 border-b border-white/5 relative z-10 text-center">
        ...
        <p className="font-light text-neutral-400 text-xs sm:text-sm max-w-lg mx-auto mb-16">
          Toggle retargeting modes to compare standard broad campaigns against optimized customer acquisition frameworks.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center text-left">
            ...
            <div className="space-y-5 font-mono text-[11px]">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${performanceMode === 'optimized' ? 'text-amber-400' : 'text-rose-400'}`}>
                    {performanceMode === 'optimized' ? '$0.85 (Ultra Low)' : '$4.50 (High Cost)'}
                  </span>
                </div>
                <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                  <motion.div 
                    animate={{ width: performanceMode === 'optimized' ? '18%' : '90%' }}
                    transition={{ type: "spring", stiffness: 100 }}
                    className={`h-full ${performanceMode === 'optimized' ? 'bg-amber-500' : 'bg-rose-500'}`}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${performanceMode === 'optimized' ? 'text-amber-400' : 'text-rose-400'}`}>
                    {performanceMode === 'optimized' ? '6.8% CTR' : '1.2% CTR'}
                  </span>
                </div>
                <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                  <motion.div 
                    animate={{ width: performanceMode === 'optimized' ? '90%' : '15%' }}
                    transition={{ type: "spring", stiffness: 100 }}
                    className={`h-full ${performanceMode === 'optimized' ? 'bg-amber-500' : 'bg-rose-500'}`}
                  />
                </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${performanceMode === 'optimized' ? 'text-amber-400' : 'text-rose-400'}`}>
                    {performanceMode === 'optimized' ? '5.2x ROI' : '1.1x ROI'}
                  </span>
                </div>
                <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                  <motion.div 
          ...
          <div className="lg:col-span-7 bg-[#09090d]/80 border border-white/5 rounded-3xl p-8 h-80 flex flex-col justify-between relative overflow-hidden backdrop-blur-md">
            ...
            <div className="flex justify-between items-center pb-4 border-b border-white/5 relative z-10">
              <span className="font-mono text-[9px] uppercase tracking-widest text-neutral-500">Live Funnel Leakage Test</span>
              <span className={`font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 rounded ${
                performanceMode === 'optimized' ? 'bg-amber-500/10 text-amber-400' : 'bg-rose-500/10 text-rose-400'
              }`}>
                {performanceMode === 'optimized' ? 'High Conversion Flow' : 'Budget Drainage'}
              </span>
            </div>
            ...
            <div className="relative flex-1 flex items-center justify-center">
              <motion.div
                animate={performanceMode === 'optimized' 
                  ? { rotate: [0, 360], scale: [0.9, 1.1, 0.9] } 
                  : { rotate: [0, 10, -10, 0], scale: [0.9, 0.92, 0.9] }
                }
            ...
            <div className="text-center font-mono text-[9px] text-neutral-500 relative z-10">
              // The golden box represents relative landing page bounce/exit rates.
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-28 border-b border-white/5 relative z-10 text-center">
        ...
        <p className="font-light text-neutral-400 text-xs sm:text-sm max-w-lg mx-auto mb-16">
          Toggle campaign networks to trace data pixels and conversion tracking hooks on the diagram.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 text-left items-stretch">
          ...
          <div className="lg:col-span-4 flex flex-col gap-4 justify-center">
            {[
              { key: 'seo', label: 'Lighthouse Page Optimization', desc: 'Accelerates landing page response rates' },
              { key: 'ads', label: 'Meta Pixel & API Retargeting', desc: 'Secure customer retargeting cookie setups' },
              { key: 'retarget', label: 'Google Search Campaigning', desc: 'High-yield keyword configuration' },
              { key: 'analytics', label: 'ROI Analytics Dashboard', desc: 'Bespoke reporting portal updates' }
            ].map(item => (
              <div 
                key={item.key}
          ...
          <div className="lg:col-span-8 bg-[#07070b]/60 border border-white/5 rounded-3xl p-6 relative overflow-hidden backdrop-blur-md min-h-[340px] flex items-center justify-center">
            ...
            <svg viewBox="0 0 600 300" className="w-full h-full max-w-[540px] overflow-visible">
              ...
              <line x1="80" y1="150" x2="200" y2="150" stroke="#f59e0b" strokeWidth="1.5" />
              ...
              <circle cx="80" cy="150" r="25" fill="#09090d" stroke="#f59e0b" strokeWidth="2" />
              <text x="80" y="153" fill="white" fontSize="8" fontFamily="monospace" textAnchor="middle">VISITOR</text>

              <circle cx="200" cy="150" r="25" fill="#09090d" stroke="#f59e0b" strokeWidth="2" />
              <text x="200" y="153" fill="white" fontSize="8" fontFamily="monospace" textAnchor="middle">PIXEL LOG</text>

              <circle cx="350" cy="80" r="25" fill="#09090d" stroke={features.seo ? "#fbbf24" : "rgba(255,255,255,0.05)"} strokeWidth="2" />
              <text x="350" y="83" fill={features.seo ? "white" : "#444"} fontSize="8" fontFamily="monospace" textAnchor="middle">SEO SPEED</text>

              <circle cx="350" cy="150" r="25" fill="#09090d" stroke="#10b981" strokeWidth="2" />
              <text x="350" y="153" fill="white" fontSize="8" fontFamily="monospace" textAnchor="middle">METRICS</text>

              <circle cx="350" cy="220" r="25" fill="#09090d" stroke={features.retarget ? "#a855f7" : "rgba(255,255,255,0.05)"} strokeWidth="2" />
              <text x="350" y="223" fill={features.retarget ? "white" : "#444"} fontSize="8" fontFamily="monospace" textAnchor="middle">KEYWORD</text>

              <circle cx="500" cy="80" r="25" fill="#09090d" stroke={features.ads ? "#ec4899" : "rgba(255,255,255,0.05)"} strokeWidth="2" />
              <text x="500" y="83" fill={features.ads ? "white" : "#444"} fontSize="8" fontFamily="monospace" textAnchor="middle">META PIX</text>

              <circle cx="500" cy="220" r="25" fill="#09090d" stroke={features.analytics ? "#6366f1" : "rgba(255,255,255,0.05)"} strokeWidth="2" />
              <text x="500" y="223" fill={features.analytics ? "white" : "#444"} fontSize="8" fontFamily="monospace" textAnchor="middle">ROI RPT</text>
      ...
      <section className="max-w-7xl mx-auto px-4 py-28 border-b border-white/5 relative z-10 text-center">
        ...
        <p className="font-light text-neutral-400 text-xs sm:text-sm max-w-lg mx-auto mb-16">
          Toggle options below to preview analytics reporting screens inside our dashboard container.
        </p>

        {/* Tab triggers */}
        <div className="flex justify-center gap-4 mb-10">
          {[
            { id: 'roi', label: 'ROAS Performance Summary' },
            { id: 'adset', label: 'Ad Account Placement' },
            { id: 'conversion', label: 'Conversion Funnel Stages' }
          ].map(tab => (
            <button
              key={tab.id}
              ...
              className={`px-5 py-2 rounded-xl text-xs font-mono font-medium tracking-wide border transition-all ${
                mockupTab === tab.id 
                  ? 'bg-amber-500 text-black border-amber-500 font-bold shadow-lg' 
            ...
            <AnimatePresence mode="wait">
              {mockupTab === 'roi' && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-[#050302] text-left p-12 flex flex-col justify-between"
                >
                  <div className="space-y-4">
                    <span className="font-mono text-[9px] uppercase tracking-widest text-amber-500">// ROAS REPORT METRICS</span>
                    <h3 className="font-display text-2xl font-bold text-white tracking-tight leading-tight">Average Return: 5.2x ROAS Achieved</h3>
                    <p className="font-light text-neutral-400 text-xs sm:text-sm max-w-sm">Consolidated performance tracking metrics verifying customer lifetime values.</p>
                  </div>
                  <div className="w-24 h-10 border border-amber-500/20 rounded flex items-center justify-center text-amber-400 font-mono text-[10px]">
                    ROAS_STABLE
                  </div>
                </motion.div>
              )}
              ...
              {mockupTab === 'adset' && (
                ...
                >
                  <div className="space-y-4">
                    <span className="text-amber-400 text-[9px] uppercase tracking-widest">// AD PLACEMENTS</span>
                    <h3 className="text-2xl font-bold text-white tracking-tight leading-tight">Reach: 840,000 Impressions</h3>
                    <div className="h-28 border border-white/5 rounded-xl bg-white/[0.01] flex items-center justify-center text-neutral-600 text-[10px]">
                      [Ad placement graph display]
                    </div>
                  </div>
                </motion.div>
              )}

              {mockupTab === 'conversion' && (
                <motion.div
                  key="conversion"
                  initial={{ opacity: 0, scale: 0.98 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.98 }}
                  className="absolute inset-0 bg-[#0e0904] text-left p-12 flex flex-col justify-between font-mono"
                >
                  <div className="space-y-4">
                    <span className="text-[8px] uppercase tracking-widest text-amber-400">// FUNNEL PIXEL SUMMARY</span>
                    <h3 className="text-xl font-normal text-white">Visitor Conversion Ledger</h3>
                    <div className="space-y-2 text-[10px] text-neutral-400">
                      <div>- Landing Page Visitors: 120,400</div>
                      <div>- Product View Clicks: 34,200</div>
                      <div>- Completed Conversions: 8,150 (6.7% Rate)</div>
                    </div>
                  </div>
                </motion.div>
              )}
