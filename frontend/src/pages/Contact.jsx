  ...
  useEffect(() => {
    const updateClocks = () => {
      ...
      activeOffices.forEach((office) => {
        slots[office.slug || office.id] = new Date().toLocaleTimeString('en-US', {
    ...
    <div className="relative text-neutral-300 font-sans overflow-hidden">
      ...
      <div className="text-center mb-20 pt-8 relative z-10">
        ...
        <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white mb-6">
          {content?.contact_settings?.title || "Let's Create Together"}
        </h1>
        <p className="font-light text-neutral-400 text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
          {content?.contact_settings?.subtitle || "Have a project in mind, want to inquire about custom solutions, or simply want to say hello? We'd love to hear from you."}
        </p>
      </div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {(content?.contact_settings?.stats || [
            ...
            const isEmerald = stat.icon === 'time';
            const isBlue = stat.icon === 'globe';
            const hoverBorder = isEmerald ? 'rgba(16, 185, 129, 0.3)' : isBlue ? 'rgba(59, 130, 246, 0.3)' : 'rgba(168, 85, 247, 0.3)';
            const iconBg = isEmerald ? 'bg-emerald-500/10 border-emerald-500/20' : isBlue ? 'bg-blue-500/10 border-blue-500/20' : 'bg-purple-500/10 border-purple-500/20';
            const iconColor = isEmerald ? 'text-emerald-400' : isBlue ? 'text-blue-400' : 'text-purple-400';
      ...
      {/* ══════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════ */}
      <motion.div 
        key={activeOffices.map(o => o.slug || o.id).join(',')}
        variants={containerVariants}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
        ...
        <div className="lg:col-span-7 flex flex-col gap-8">
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-6 pb-6 border-b border-white/5">
            <div className="flex-1">
              ...
              <div className="space-y-2">
                <a href={`mailto:${content?.contact_settings?.email_hello || "hello@harikrushndigiverse.com"}`} className="flex items-center gap-3 text-neutral-300 hover:text-white transition-colors group">
                  ...
                  <span className="font-light text-xs">{content?.contact_settings?.email_hello || "hello@harikrushndigiverse.com"}</span>
                </a>
                <a href={`mailto:${content?.contact_settings?.email_join || "join@harikrushndigiverse.com"}`} className="flex items-center gap-3 text-neutral-300 hover:text-white transition-colors group">
                  ...
                  <span className="font-light text-xs">{content?.contact_settings?.email_join || "join@harikrushndigiverse.com"}</span>
                </a>
                <a href={`tel:${(content?.contact_settings?.phone || "+91 98765 43210").replace(/\s/g, '')}`} className="flex items-center gap-3 text-neutral-300 hover:text-white transition-colors group">
                  ...
                  <span className="font-light text-xs">{content?.contact_settings?.phone || "+91 98765 43210"}</span>
                </a>
              </div>
            </div>
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  {
                    name: 'LinkedIn',
                    url: content?.site_settings?.social_links?.find(s => s.platform.toLowerCase() === 'linkedin')?.url || 'https://linkedin.com',
                  ...
                  {
                    name: 'Instagram',
                    url: content?.site_settings?.social_links?.find(s => s.platform.toLowerCase() === 'instagram')?.url || 'https://instagram.com',
                  ...
                  {
                    name: 'Twitter / X',
                    url: content?.site_settings?.social_links?.find(s => s.platform.toLowerCase() === 'twitter' || s.platform.toLowerCase() === 'twitter / x')?.url || 'https://twitter.com',
                  ...
                  {
                    name: 'YouTube',
                    url: content?.site_settings?.social_links?.find(s => s.platform.toLowerCase() === 'youtube')?.url || 'https://youtube.com',
                  ...
                  <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2.5 p-2.5 rounded-xl bg-white/[0.02] border border-white/5 transition-all duration-300 group ${social.hoverClass}`}>
                    {social.icon}
          ...
          {activeOffices.map((office) => (
            ...
            >
              ...
              <div className="flex justify-between items-center border-t border-white/5 pt-5 text-xs font-mono">
                <a href={`tel:${(office.phone || '').replace(/\s/g, '')}`} className="text-neutral-400 hover:text-white hover:underline transition-colors">{office.phone || 'N/A'}</a>
                <div className="flex items-center gap-2 text-neutral-300">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  <span>LOCAL TIME: {timeSlots[office.slug || office.id] || '--:--:--'}</span>
                </div>
              </div>
            </motion.div>
          ))}
          ...
          <motion.div variants={itemVariants} className="bg-[#050508]/40 border border-white/5 rounded-3xl p-7">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(content?.contact_settings?.business_hours || [
              ...
              ]).filter(h => h.active !== false).map(h => (
                <div key={h.day} className={`flex items-center justify-between p-3.5 rounded-xl border transition-all duration-300 ${h.color || 'text-neutral-300 bg-white/[0.01] border-white/5'}`}>
          ...
          {/* WhatsApp Quick Chat */}
          {(content?.contact_settings?.whatsapp_numbers || [
            { label: "Chat on WhatsApp", subtext: "Instant response • Available 24/7", number: "919876543210" }
          ]).map((wa, idx) => (
            <motion.a 
              key={idx}
              variants={itemVariants}
              whileHover={{ scale: 1.01 }}
              href={`https://wa.me/${(wa.number || '').replace(/\+/g, '').replace(/\s/g, '')}?text=Hi%20HariKrushn%20Team%2C%20I%20am%20interested%20in%20your%20services.`} 
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center gap-4 p-5 rounded-3xl bg-[#25D366]/5 border border-[#25D366]/15 hover:border-[#25D366]/40 hover:shadow-[0_0_30px_rgba(37,211,102,0.15)] transition-all group mb-3"
            >
              ...
              <div>
                <h4 className="font-display font-bold text-[#25D366] text-sm group-hover:text-[#2be672] transition-colors">{wa.label}</h4>
                <p className="font-mono text-[8px] uppercase tracking-widest text-neutral-500 mt-0.5">{wa.subtext}</p>
              </div>
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {sent ? (
              ...
              <form key="form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {(content?.contact_settings?.form_fields || [
                  { "id": "name", "label": "Name", "type": "text", "placeholder": "Your name", "required": true, "halfWidth": true },
                  { "id": "phone", "label": "Phone", "type": "tel", "placeholder": "+91 XXXXX XXXXX", "required": false, "halfWidth": true },
                  { "id": "email", "label": "Email", "type": "email", "placeholder": "your@email.com", "required": true, "halfWidth": false },
                  { "id": "service", "label": "Service Interested In", "type": "select", "placeholder": "Select service...", "required": false, "halfWidth": true, "options": ["Web Development", "AI Solutions", "UI/UX Design", "Digital Marketing", "Other"] },
                  { "id": "budget", "label": "Budget Range", "type": "select", "placeholder": "Select budget...", "required": false, "halfWidth": true, "options": ["Under ₹50k", "₹50k - ₹1L", "₹1L - ₹5L", "₹5L+", "Not Decided"] },
                  { "id": "message", "label": "Project Brief", "type": "textarea", "placeholder": "Describe your project, timeline, and requirements", "required": true, "halfWidth": false }
                ]).map((field) => {
                  const isHalf = field.halfWidth;
                  const colSpan = isHalf ? "sm:col-span-1" : "sm:col-span-2";
                  
                  return (
                    <div key={field.id} className={`flex flex-col gap-1.5 text-left ${colSpan}`}>
                      <label htmlFor={`contact-${field.id}`} className="font-mono text-[9px] uppercase tracking-widest text-neutral-500">
                      ...
                      {field.type === 'textarea' ? (
                        <textarea 
                          ...
                          className="bg-black/60 border border-white/5 rounded-xl px-4 py-3 text-xs text-white placeholder-neutral-600 focus:outline-none focus:border-emerald-500/40 focus:bg-emerald-500/[0.02] focus:shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all resize-none w-full" 
                        />
                      ) : field.type === 'select' ? (
                        <div className="relative">
                          <select 
                            ...
                            value={formData[field.id] || ''} 
                            onChange={(e) => setFormData({ ...formData, [field.id]: e.target.value })} 
                            className="bg-black/60 border border-white/5 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-emerald-500/40 focus:bg-emerald-500/[0.02] focus:shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all appearance-none cursor-pointer w-full"
                          >
                            <option value="" className="bg-black">{field.placeholder || 'Select option...'}</option>
                            {(field.options || []).map(opt => (
                              <option key={opt} value={opt} className="bg-black">{opt}</option>
                            ))}
                          </select>
                          <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-neutral-500 text-[10px]">
                            ▼
                          </div>
                        </div>
                      ) : (
                        <input 
                          ...
                          onChange={(e) => setFormData({ ...formData, [field.id]: e.target.value })} 
                          placeholder={field.placeholder} 
                          className="bg-black/60 border border-white/5 rounded-xl px-4 py-3 text-xs text-white placeholder-neutral-600 focus:outline-none focus:border-emerald-500/40 focus:bg-emerald-500/[0.02] focus:shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all w-full" 
                        />
                      )}
                    </div>
                  );
                ...
                <div className="sm:col-span-2">
                  <button 
                    type="submit" 
                    disabled={submitting}
                    className={`bg-white text-black hover:bg-emerald-400 hover:text-black hover:shadow-[0_0_20px_rgba(52,211,153,0.4)] px-6 py-4 rounded-xl text-[10px] font-mono font-bold uppercase tracking-widest transition-all duration-300 mt-2 shadow-md cursor-pointer w-full ${submitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
      ...
      {/* ══════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════ */}
      {(content?.contact_settings?.contact_map?.show !== false) && (
        <motion.section 
        ...
        >
          <div className="text-center mb-10">
            <span className="font-mono text-[9px] uppercase tracking-widest text-neutral-500 block mb-3">
              {content?.contact_settings?.contact_map?.subtitle || "// Visit Our Office"}
            </span>
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-white tracking-tight mb-3">
              {content?.contact_settings?.contact_map?.title || "Our Location"}
            </h2>
            <p className="font-light text-neutral-400 text-xs sm:text-sm max-w-lg mx-auto">
              {content?.contact_settings?.contact_map?.address || "Silver Trade Center, 501 & 502, near Pragati IT Park, Mota Varachha, Surat, Gujarat 394101"}
            </p>
          </div>
          <div className="rounded-3xl overflow-hidden border border-white/10 shadow-2xl h-80 sm:h-[450px] relative bg-neutral-900 group">
            <iframe
              src={content?.contact_settings?.contact_map?.map_iframe_url || "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3719.5!2d72.8614!3d21.2254!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3be04f3f2b23c6b1%3A0x1234567890abcdef!2sSilver%20Trade%20Center%2C%20Mota%20Varachha%2C%20Surat%2C%20Gujarat%20394101!5e0!3m2!1sen!2sin!4v1700000000000!5m2!1sen!2sin"}
              width="100%"
              height="100%"
              ...
              title="HariKrushn Digiverse Office"
            />
            {/* Address overlay */}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-display font-bold text-white text-sm mb-1">
                    {content?.contact_settings?.contact_map?.card_title || "HariKrushn Digiverse LLP"}
                  </h4>
                  <p className="font-light text-neutral-400 text-[11px] leading-relaxed">
                    {content?.contact_settings?.contact_map?.address || "Silver Trade Center, 501 & 502, near Pragati IT Park, Mota Varachha, Surat, Gujarat 394101"}
                  </p>
                  {content?.contact_settings?.contact_map?.directions_url && (
                    <a 
                      href={content?.contact_settings?.contact_map?.directions_url} 
                      target="_blank" 
                      rel="noopener noreferrer"
